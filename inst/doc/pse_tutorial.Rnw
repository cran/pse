...
\date{Version 0.3.0, November 23, 2013} 
\maketitle
...
should be investigated, what are the probability distribution functions (PDFs)
...
We are insterested in studying the effects of the parameters $r$, $K$
...
in the sensitivity analyses, containing the names of the parameters,
the distribution functions, and {\em a list containing the lists} with 
all the parameters to the distribution functions:
...
A discrete uniform distribution function, usefull for parameters that
...
\textbf{corPlot}, partial rank correlation using the function \textbf{plotprcc} and
...
to each parameter. {\em Please note} that in the current version of the package,
the pic function only works for models with one response variable, and should be
considered in {\em beta}\footnote{Future releases of the package will probably
merge the prcc and pic calls in a more friendly structure}:
...
responses from the model. In our example, we are interested
...
are to be run do not have \R installed. In order to do this, however, you 
